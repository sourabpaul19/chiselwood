...
<div class="section_header">
    <div class="d-flex align-items-center mb-2 justify-content-between">
        <h4>Lead Status Management</h4>
    </div>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
            <li class="breadcrumb-item active" aria-current="page">Lead Status Management</li>
        </ol>
    </nav>
</div>
...
<div class="row">
    <div class="col-sm-4 form-wrap">
        ...
        <form method="POST"
      action="{{ isset($status)
        ? route('admin.lead-statuses.update', $status)
    ...
    <h2>{{ isset($status) ? 'Edit' : 'Add' }} Lead Status</h2>
    ...
    <div class="form_group mb-3">
        <button class="btn btn-theme">
            {{ isset($status) ? 'Update Lead Status' : 'Add Lead Status' }}
        </button>
    </div>
</form>
    ...
    <div class="col-md-8">
        ...
        <div class="table_top_header">
            <ul class="status_list">
                <li><a href="{{ route('admin.lead-statuses.index') }}" class="{{ !request('status') ? 'active' : '' }}">All ({{ $statuses->total() }})</a></li>
                <li>|</li>
                ...
                <li>|</li>
                <li><a href="{{ route('admin.lead-statuses.trash') }}">Trash</a></li>
            </ul>
            
            {{-- Search Form --}}
            <form method="GET" action="{{ route('admin.lead-statuses.index') }}" class="search_bar ps-sm-2 d-flex gap-2">
                @if(request('status'))
                ...
                @if(request('search'))
                    <a href="{{ route('admin.lead-statuses.index') }}" class="btn">Clear</a>
                @endif
            </form>
        </div>
        ...
        <div class="table_top_header">
            <form method="GET" action="{{ route('admin.lead-statuses.index') }}" class="filter_bar pe-sm-2 d-flex gap-2">
                @if(request('search'))
...
<tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <span><a href="{{ route('admin.lead-statuses.index', ['edit' => $status->id]) }}">Edit</a></span>
                    <span>|</span>
                    <span><a href="{{ route('admin.lead-statuses.status.toggle', $status->id) }}"
                        ...
                        ">
                            ...
                            action="{{ route('admin.lead-statuses.status.toggle', $status->id) }}"
                            method="POST"
                            style="display:none;">
                            @csrf
                            @method('PATCH')
                                            ...
                                            <span>
                                                ...
                                                <form id="delete-form-{{ $status->id }}"
                                                    action="{{ route('admin.lead-statuses.destroy', $status->id) }}"
                                                    method="POST"
                                                    style="display:none;">
                                                    @csrf
                                                    @method('DELETE')
